...
if __name__ == "__main__":
    ...
    def extract_question(text: str) -> str:
        """
        Return the text between 'Question:' and the first 'Example'
        (or 'Examples'), trimmed. Case-insensitive, spans multiple lines.
        """
        m = re.search(r'(?si)\bQuestion:\s*(.*?)\n\s*Example(?:s|\b)', text)
        return m.group(1).strip() if m else ""

    # add a row to each data item that represents a unique id
    def make_map_fn(split):
        def process_fn(example, idx):
            question_raw = extract_question(example.pop("query")) + "\n" + example.pop("response")
