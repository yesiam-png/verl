...
#                    break
                ...
                if reward <= 0.2 and not training_q:  # ensure that format is always correct when not training q
                    content = "\n"
...
#                )
                ...
                if user_turns >= len(_req.split_lines) - 2 or user_turns >= self.config.max_code_lines - 1 or len(_req.get_generation_prompt_ids(self.processing_class)) >= self.config.response_length:
